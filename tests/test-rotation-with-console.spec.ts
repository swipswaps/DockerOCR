...
test('test rotation with console logging', async ({ page }) => {
  ...
  page.on('console', msg => {
    ...
    if (text.includes('[generateProcessedImage]') || text.includes('rotation') || text.includes('filters')) {
  ...
  const rotateButtons = await page.locator('button').filter({ hasText: /rotate/i }).all();
  ...
  if (await runButton.isVisible()) {
    ...
    console.log('\n📋 PROCESS LOGS IN UI:');
    console.log('─────────────────────────────────────────────────');
    const rotationLogs = processLogs.filter(log => 
      log.includes('rotation') || 
      log.includes('Rotated') ||
      log.includes('filters')
    );
