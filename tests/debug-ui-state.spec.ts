...
test('debug UI state after rotation', async ({ page }) => {
  ...
  for (let i = 0; i < allButtons.length; i++) {
    ...
    if (text?.trim() || title) {
      console.log(
        `Button ${i}: "${text?.trim() || ''}" title="${title || ''}" disabled=${disabled} visible=${visible}`
      );
    }
  }
  ...
  const runOCRButton = page
    .locator(
      'button:has-text("Run OCR"), button:has-text("Run"), button:has-text("Extract"), button:has-text("Process")'
    )
    .first();
